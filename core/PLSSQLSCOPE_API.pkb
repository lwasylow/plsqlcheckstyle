...
CREATE OR REPLACE PACKAGE BODY plssqlscope_api IS
   ...
   BEGIN
      dbms_application_info.set_module(module_name => i_modulename,
                                       action_name => NULL);
   END set_module;
   ...
   FUNCTION get_object_list(i_schemas IN varchar2_tab, i_objects IN varchar2_tab)
      RETURN t_objects IS
      ...
      CURSOR c_schemas_object IS
         SELECT t_object(o.object_name, o.owner, o.object_type)
          ...
          WHERE o.owner IN (SELECT *
                              FROM TABLE(i_schemas)) AND
                o.object_name IN (SELECT *
                                    FROM TABLE(i_objects)) AND
                o.object_type IN
                ('PROCEDURE', 'FUNCTION', 'PACKAGE BODY', 'PACKAGE') AND
                o.generated <> 'Y' AND NOT EXISTS
          (SELECT 1
                   FROM TABLE(l_excluded_list) t
                  WHERE o.object_name = NVL(t.objectname, o.object_name) AND
                        o.owner = NVL(t.objectowner, o.owner) AND
                        o.object_type = NVL(t.objecttype, o.object_type))
          ORDER BY o.owner,
                   o.object_name,
                   CASE
   ...
   BEGIN
      ...
      OPEN c_idents FOR q'[WITH globalsection AS
          (SELECT MIN(line) end_line
           ...
           AND    usage != 'CALL'
           AND    owner = :p_objectowner
           AND    object_name = :p_objectname
           AND    object_type = :p_objecttype
         ...
         identifiers AS
          (SELECT i.name
           ...
           AND :p_objectowner = i.owner AND :p_objectname = i.object_name AND
           :p_objecttype = i.object_type )
         SELECT NAME
               ,TYPE
               ,usage
               ,line
         ...
         FROM   (SELECT i.name
                 ...
                 SELECT TO_CHAR(message_number)
                       ...
                       ,NULL end_line
                 FROM   dba_errors e
                 WHERE  e.owner = :p_objectowner
                 AND    e.name = :p_objectname
                 AND    e.type = :p_objecttype)]'
         USING i_path.objectowner, i_path.objectname, i_path.objecttype, i_path.objectowner, i_path.objectname, i_path.objecttype, i_path.objectowner, i_path.objectname, i_path.objecttype;
   ...
   BEGIN
      ...
      FOR rulelist IN 1 .. l_rules.COUNT LOOP
         l_sql_base := l_sql_base_template;
         l_sql_base := REPLACE(l_sql_base,
                               '#RULENAME#',
                               l_rules(rulelist).rulename);
         l_sql_base := REPLACE(l_sql_base,
                               '#RULEDESC#',
                               l_rules(rulelist).ruledescription);
         l_sql_base := REPLACE(l_sql_base,
                               '#CATEGORY#',
                               l_rules(rulelist).category);
   ...
   BEGIN
      ...
      SELECT t_scope_result_row(object_owner,
                                object_name,
                                object_type,
                                rule_name,
                                identifier,
       ...
       (SELECT 1
               ...
               WHERE src.object_owner = NVL(ign.object_owner, src.object_owner) AND
                     src.object_name = NVL(ign.object_name, src.object_name) AND
                     src.object_type = NVL(ign.object_type, src.object_type) AND
                     src.rule_name = NVL(ign.rule_name, src.rule_name) AND
                     src.identifier = NVL(ign.identifier, src.identifier) AND
                     src.line = NVL(ign.line, src.line));
   ...
   BEGIN
      ...
      FOR object_details IN (SELECT t_object(objectname, objectowner, objecttype) details
                               FROM TABLE(l_path)) LOOP
         gather_identifiers(i_path => object_details.details);
      END LOOP;
      l_path.DELETE;
   ...
   BEGIN
      ...
      IF i_objects.COUNT = 0 THEN
         ...
         --that creates a lock
         FOR listofschemas IN 1 .. i_schemas.COUNT LOOP
   ...
   BEGIN
      ...
      IF l_reporter.reporter_name = 'HTML' THEN
         report_to_html(i_source_data  => i_source_data,
                        i_reporter_det => l_reporter,
                        o_data         => o_report);
      ELSIF l_reporter.reporter_name = 'PMD' THEN
         report_to_pmd(i_source_data  => i_source_data,
                       i_reporter_det => l_reporter,
                       o_data         => o_report);
      ...
      ELSIF l_reporter.reporter_name = 'CSVEXCL' THEN
         report_to_csvexclusions(i_source_data => i_source_data,
                                 o_data        => o_report);
      ELSE
      ...
      WHILE l_offset < l_length LOOP
         ...
         IF l_length > l_chunk_length THEN
            IF l_tagendpos = 0 THEN
               l_report_line := dbms_lob.substr(i_report_data,
                                                l_length - l_offset,
                                                l_offset);
         ...
         insert_into_buffer(i_line        => l_report_line,
                            i_line_id     => l_line,
   ...
   BEGIN
      FOR textlines IN 1 .. l_text_tab.COUNT LOOP
   ...
   PROCEDURE run_and_print_reporter(i_reporter    IN t_reporter_rec,
      ...
      l_data CLOB;
   BEGIN
      get_report_data(i_reporter    => i_reporter,
                      i_source_data => i_source_data,
                      o_report      => l_data);
   ...
   BEGIN
      run_scope(i_schemas => i_schemas,
                i_objects => i_objects,
                o_results => l_result_set);
      run_and_print_reporter(i_reporter    => i_reporter,
   ...
   BEGIN
      ...
      FOR reporters IN i_reporter.FIRST .. i_reporter.LAST LOOP
   ...
   BEGIN
      set_module;
      run_scope(i_schemas => l_schema,
                i_objects => l_objects,
                o_results => l_result_set);
   
      FOR resultset IN 1 .. l_result_set.COUNT LOOP
   ...
   BEGIN
      set_module;
      run_scope_with_print(i_schemas  => l_schema,
                           i_objects  => l_objects,
                           i_reporter => i_reporter);
   END run;

   FUNCTION run(i_schema IN VARCHAR2 DEFAULT NULL, i_object IN VARCHAR2)
      RETURN t_scope_result_rows
      PIPELINED IS
      l_schema varchar2_tab := varchar2_tab(coalesce(i_schema, g_current_schema));
